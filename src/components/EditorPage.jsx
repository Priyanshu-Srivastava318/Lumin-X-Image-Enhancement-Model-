...
const EditorPage = ({ image, onBack, userId }) => {
  ...
  const getCurrentParams = () => {
    switch (algorithm) {
      case 'retinex':      return retinexParams;
      case 'clahe':        return claheParams;
      case 'dark_channel': return darkChannelParams;
      case 'lime':         return limeParams;
      default:             return {};
    }
  };
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="range" min="5" max="25" step="2"
          ...
          <div className="flex gap-3">
            {saveSuccess && (
              <div className="flex items-center gap-2 text-green-400 bg-green-900/30 px-4 py-2 rounded-lg border border-green-700 animate-pulse">
                <Check className="w-5 h-5" /> Saved to Database
              </div>
            )}
            {enhancedImage && (
              <>
            ...
            {metrics && (
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-4 border-2 border-green-600/50 shadow-2xl">
                <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                  <Award className="w-5 h-5 text-yellow-400" /> Quality Assessment
                </h3>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={m.label}>
                      ...
                      {m.key && showMetricInfo === m.key && (
                        <div className="text-xs text-slate-400 p-2 bg-slate-900 rounded mt-1">
                          {metricExplanations[m.key === 'illum' ? 'illuminationGain' : m.key]?.interpretation}
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm text-slate-300 mb-2 flex justify-between items-center">
                      ...
                      <span className="text-xs bg-slate-900 px-2 py-1 rounded">Avg Brightness: {metrics?.originalBrightness}</span>
                    </div>
                  ...
                  <div>
                    <div className="text-sm text-slate-300 mb-2 flex justify-between items-center">
                      ...
                      <span className="text-xs bg-green-900 px-2 py-1 rounded text-green-300">Avg Brightness: {metrics?.enhancedBrightness}</span>
                    </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-slate-300">
                  ...
                  <ul className="mt-2 space-y-1 text-xs list-disc list-inside text-slate-400">
                    ...
                    <li>Smart detection automatically adjusts enhancement intensity</li>
